"""Config flow for Blue Connect Bluetooth integration."""
...
from blueconnectha_ble.const import BLUE_CONNECT_MFR_ID
...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    """Handle a config flow for Blue Connect Bluetooth."""
        ...
        if discovery := self._discovery_info:
            ...
            for discovery in async_discovered_service_info(self.hass):
                if (
                    ...
                    or BLUE_CONNECT_MFR_ID not in discovery.manufacturer_data
                ):
                    continue
                self._discovered_devices[discovery.address] = discovery
    ...
    ) -> BlueConnectBLEOptionsFlowHandler:
        """Get the options flow for this handler."""
        return BlueConnectBLEOptionsFlowHandler(config_entry)


class BlueConnectBLEOptionsFlowHandler(config_entries.OptionsFlow):
    """Handle BlueConnectBLE options."""

    def __init__(self, config_entry: config_entries.ConfigEntry) -> None:
        """Initialize BlueConnectBLE options flow."""
        self.entry = config_entry

    async def async_step_init(
        self, user_input: dict[str, Any] | None = None
    ) -> FlowResult:
        """Manage the BlueConnectBLE options."""
        return await self.async_step_device_options()

    async def async_step_device_options(
        self, user_input: dict[str, Any] | None = None
    ) -> FlowResult:
        """Manage the BlueConnectBLE devices options."""
        if user_input is not None:
            return self.async_create_entry(
